...
class Snake_Rander:
    ...
    def game_render(self,game_1,game_2=None):
        ...
        if game_1['over']:
            if self.mode==1:
                show_over(game_1['state'][4],'l')
            else:
                show_over(game_1['state'][4])
        else:
            show_object(game_1['board'])

        show_score(game_1['state'][4],game_1['count'])
        if self.mode==1:
            if game_2['over']:
                show_over(game_2['state'][4],'r')
            else:
                show_object(game_2['board'],self.window_size+50)
            show_score(game_2['state'][4],game_2['count'],'r')
